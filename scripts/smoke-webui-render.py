...
def exercise_app(context, url: str, viewport: dict[str, int], label: str) -> None:
    ...
    # drill into the experiment detail (GET /api/experiments/{slug}) + ledger
    ...
    page.get_by_placeholder("query").fill("webui")
    ...
    # per-item detail drill-down (GET /api/work_items/{public_id})
    ...
    # tree view (GET /api/work_items/tree?root=PLAN-1)
    page.get_by_placeholder("plan id").fill("PLAN-1")
    page.get_by_text("tree", exact=True).click()
