//! Emacs Org-mode renderer. `*`/`**` headings, pipe tables (Org realigns them
//! on open), `#+BEGIN_QUOTE` blocks for recommended fixes. Same unicode glyphs
//! and sparklines as Markdown.
...
pub fn render(report: &QualityReport) -> String {
    ...
    s.push_str(&format!("#+TITLE: Quality Report: {}\n", report.project));
    s.push_str(&format!(
        "#+SUBTITLE: {} (GPA {}) · ORR {} · pgmcp {}\n",
    ...
    s.push_str(&format!(
        "* Reproduce\n  : pgmcp tool quality_report project={} format=org\n",
        report.project
    ));
    s
}
