...
pub fn parse_perf_report(text: &str) -> Vec<PerfReportEntry> {
    ...
    for raw in text.lines() {
        ...
        let (children_pct, self_pct, rest): (f64, f64, &[&str]) = if let Some(second) =
            rest_after_first.first()
...
";
        ...
        assert!(
            s.top_frames
                .iter()
                .any(|f| f.function == "grow_table" && f.bytes == 2_097_152),
            "frames: {:?}",
            s.top_frames
        );
...
";
        ...
        assert!(
            s.top_frames
                .iter()
                .any(|f| f.function == "build_index" && f.bytes == 1_048_576),
            "frames: {:?}",
            s.top_frames
        );
    }
