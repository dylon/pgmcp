...
/// All fields used by MCP tool handlers in server.rs (accessed via graph node weights).
#[derive(Debug, Clone)]
pub struct FileNode {
    pub file_id: i64,
    pub relative_path: String,
    #[allow(dead_code)]
    // Used in MCP tool handlers via petgraph node weights (compiler can't trace through #[tool_router] dispatch)
    pub language: String,
...
impl EdgeType {
    #[allow(dead_code)] // Used in MCP tool handlers (compiler can't trace through #[tool_router] dispatch)
    ...
    #[allow(clippy::should_implement_trait)] // Returns Option, not Result; not the std FromStr trait.
