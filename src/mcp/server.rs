...
impl McpServer {
    ...
    async fn dispatch_named(
        ...
        dispatch_tool!(self, name, args, {
            ...
            "work_item_topics"       => work_item_topics(WorkItemTopicsParams) in tool_corpus_topics,
            "commit_topics"          => commit_topics(CommitTopicsParams) in tool_corpus_topics,
            "prompt_topics"          => prompt_topics(PromptTopicsParams) in tool_corpus_topics,
            "topic_scoped_search"    => topic_scoped_search(TopicScopedSearchParams) in tool_topic_apps2,
            "topic_quality_forecast" => topic_quality_forecast(TopicQualityForecastParams) in tool_topic_apps2,
            "doc_code_topic_alignment" => doc_code_topic_alignment(DocCodeTopicAlignmentParams) in tool_topic_apps2,
            "topic_experiment_map"   => topic_experiment_map(TopicExperimentMapParams) in tool_topic_apps2,
            "topic_drift_warning"    => topic_drift_warning(TopicDriftWarningParams) in tool_topic_apps3,
            "topic_ownership_forecast" => topic_ownership_forecast(TopicOwnershipForecastParams) in tool_topic_apps3,
            ...
            "common_dependency"         => common_dependency(CommonDependencyParams) in tool_category2,
            "integration_point"         => integration_point(IntegrationPointParams) in tool_category2,
            "functorial_impact"         => functorial_impact(FunctorialImpactParams) in tool_category2,
            "effect_functor"            => effect_functor(EffectFunctorParams) in tool_category3,
            "naturality_gap"            => naturality_gap(NaturalityGapParams) in tool_category3,
            "colimit_view"              => colimit_view(ColimitViewParams) in tool_category3,
            ...
            "cross_project_cve_exposure" => cross_project_cve_exposure(CrossProjectCveExposureParams) in tool_cross_project_cve_exposure,
