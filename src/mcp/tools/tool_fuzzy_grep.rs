...
pub async fn run(ctx: &SystemContext, params: FuzzyGrepParams) -> Result<CallToolResult, McpError> {
    ...
    let lines: Vec<&str> = params.haystack.iter().map(|s| s.as_str()).collect();
    let dict: DynamicDawgChar<()> = DynamicDawgChar::from_terms(lines);
    ...
    json_result(&json!({
        "query": params.query,
        "max_distance": max_d,
        "matches": hits.into_iter().map(|(t, d)| json!({"line": t, "distance": d}))
            .collect::<Vec<_>>(),
    }))
}
