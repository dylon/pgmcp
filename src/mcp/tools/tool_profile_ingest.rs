...
pub async fn tool_profile_ingest(
    ...
    debug!(tool = "profile_ingest", project, kind, limit, "MCP tool invoked");
    ...
    resolved.sort_by(|a, b| {
        let pa: f64 = a["priority_score"].as_str().and_then(|s| s.parse().ok()).unwrap_or(0.0);
        let pb: f64 = b["priority_score"].as_str().and_then(|s| s.parse().ok()).unwrap_or(0.0);
