...
#[tokio::test]
async fn doc_coverage_gaps_rejects_empty_project() {
    let server = server_with_mock(MockDbClient::new());
    let result = server
        .call_tool_cli("doc_coverage_gaps", serde_json::json!({"project": "   "}))
        .await;
    assert!(result.is_err(), "blank project names must be rejected");
}

#[tokio::test]
async fn doc_coverage_gaps_rejects_duplicate_project_display_names() {
    ...
    let suffix = std::time::SystemTime::now()
        ...
        .as_nanos();
    let name = format!("doc-coverage-dup-{suffix}");
    ...
    for idx in 0..2 {
        ...
        .bind(format!("/tmp/doc-coverage-dup-{suffix}-{idx}"))
        .bind(format!("/tmp/doc-coverage-dup-{suffix}-{idx}/project"))
        .bind(&name)
        .execute(db.pool())
        .await
    ...
    let result = server
        .call_tool_cli("doc_coverage_gaps", serde_json::json!({"project": name}))
        .await
