...
fn empty_lattice_returns_empty_path_zero_cost() {
    let lat = build_correction_lattice(&[], &[], 1.0);
...
fn identity_path_preserves_input_tokens() {
    ...
    let lat = build_correction_lattice(&tokens, &cands, 1.0);
    let out = viterbi_best(&lat).expect("viterbi");
    assert_eq!(
        out.viterbi_path,
...
fn viterbi_is_deterministic_for_identical_calls() {
    ...
    let lat1 = build_correction_lattice(&tokens, &cands, 1.0);
    let lat2 = build_correction_lattice(&tokens, &cands, 1.0);
